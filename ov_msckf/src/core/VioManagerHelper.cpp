...
void VioManager::initialize_with_prior(Eigen::Matrix<double, 17, 1> imustate, 
  ...
  // Set initial covariance based on provided standard deviations
  std::vector<std::shared_ptr<ov_type::Type>> order = {state->_imu};
  Eigen::MatrixXd Cov = Eigen::MatrixXd::Identity(state->_imu->size(), state->_imu->size());
  // print imu size
  Cov.block<3,3>(0,0) = std::pow(rot_std, 2) * Eigen::Matrix3d::Identity();      // q_GtoI (attitude)
  Cov.block<3,3>(3,3) = std::pow(pos_std, 2) * Eigen::Matrix3d::Identity();      // p_IinG (position)
  Cov.block<3,3>(6,6) = std::pow(vel_std, 2) * Eigen::Matrix3d::Identity();      // v_IinG (velocity)
  Cov.block<3,3>(9,9) = std::pow(gyro_bias_std, 2) * Eigen::Matrix3d::Identity();// bias_g (gyro bias)
